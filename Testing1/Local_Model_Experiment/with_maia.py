...
with_maia.py
------------------
Flow: FEN -> Maia gives the best move -> Qwen is told the FEN + the move,
...
Usage:
    python with_maia.py "r1bqkbnr/pppp1ppp/2n5/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R b KQkq - 4 4"
    python with_maia.py --file positions.json
    python with_maia.py "FEN1" "FEN2" --file more_positions.json
    python with_maia.py "<FEN>" --maia-model /path/to/maia --model Qwen/Qwen2.5-3B-Instruct
"""
...
from common import get_best_move_maia, load_model, generate, extract_json, DEFAULT_MODEL_NAME, CHESS_CONCEPTS
...
def main():
    parser = argparse.ArgumentParser(description="Maia-assisted concept identification")
    ...
    parser.add_argument("--maia-model", default=None, help="Path to maia binary")
    parser.add_argument("--model", default=DEFAULT_MODEL_NAME, help="HF model name")
    parser.add_argument("--think-time", type=float, default=0.5, help="Seconds for Maia to think")
    parser.add_argument("--output", default="results_with_maia.json", help="Path to JSON results file")
    ...
    for i, fen in enumerate(positions, 1):
        ...
        # Get Maia's best move
        print("  Asking Maia for the best move...")
        try:
            uci, san, score = get_best_move_maia(fen, args.maia_model, args.think_time)
        except Exception as e:
            print(f"  ERROR: Maia failed: {e}")
            record = {
        ...
        print(f"  Maia says: {san} ({uci})  |  eval: {score}")
